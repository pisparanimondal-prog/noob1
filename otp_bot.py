...
_last_relogin = {}             # api_id → unix timestamp of last relogin attempt
_relogin_failures = {}         # api_id → consecutive failed relogin count
...
def _trigger_auto_relogin(api_id, api_cfg, logger):
    ...
    with _relogin_lock:
        ...
        if fail_count >= 3:
            ...
            if remaining > 0:
                logger.warning(
                    f'Auto-relogin paused for {api_id} — {fail_count} consecutive failures. '
                    f'Resuming in {remaining//60}m {remaining%60}s. '
                    f'Update credentials via Edit API → Edit Username & Password.'
                )
                return False
    ...
    def _do_relogin():
        ...
        try:
            ...
            try:
                ...
                if fail_count >= 3:
                    msg = (
                        f'🔴 <b>Auto-Login Paused — {api_name}</b>\n\n'
                        f'Failed <b>{fail_count} times</b> in a row.\n'
                        f'Auto-login is paused for <b>30 minutes</b> to prevent IP blocking.\n\n'
                        f'👉 <b>Action needed:</b> Check your panel credentials and update them via:\n'
                        f'Admin Panel → My APIs → ✏️ Edit → Edit Username & Password\n\n'
                        f'Error: <code>{str(e)[:150]}</code>'
                    )
                else:
                    msg = (
                        f'❌ <b>Auto-Login Failed ({fail_count}/3) — {api_name}</b>\n\n'
                        f'Error: <code>{str(e)[:150]}</code>\n\n'
...
def format_message(number, service, raw_text, otp_code, api_name):
    ...
    otp_line = f'\n🔑 <code>{html_escape(str(otp_code))}</code>' if otp_code else ''
    return (
...
def api_worker(api_id):
    ...
    while _running:
        try:
            ...
            # Session expired — empty / non-JSON body returned by panel
            if data.get('_session_expired'):
                ...
                update_api_status(api_id, {
                    ...
                    'status': 'error',
                    'last_check': datetime.now().isoformat(),
                    'error': 'Session expired — auto re-login triggered',
                    'otps_sent': _api_status.get(api_id, {}).get('otps_sent', 0),
                })
            ...
            if retry_after > 0:
                ...
                # After 3 consecutive drops, attempt auto-relogin as well — it costs
